...
esp_err_t pcf85063a_set_time(const struct tm *time)
{
    ...
    uint32_t year = (uint32_t)time->tm_year + 1900;
    uint32_t month = (uint32_t)time->tm_mon + 1;
    time_buf[4] = getDayOfWeek(time->tm_mday, month, year) & PCF85063A_WEEKDAYS_MASK;
    time_buf[5] = dec_to_bcd(month) & PCF85063A_MONTHS_MASK;
    time_buf[6] = dec_to_bcd(year % 100);
