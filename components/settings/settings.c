...
static uint8_t sound_volume = 80;
...
static bool settings_write_json(void)
{
    ...
    cJSON_AddNumberToObject(root, "sound_volume", sound_volume);
    cJSON_AddBoolToObject(root, "always_on", always_on);
...
static bool settings_read_json(void)
{
    ...
    if (cJSON_IsNumber(j)) sound_volume = j->valueint;
    j = cJSON_GetObjectItem(root, "always_on");
    if (cJSON_IsBool(j)) always_on = cJSON_IsTrue(j);
...
void settings_init(void) {
    ...
    if (rtc_get_time(&time) == ESP_OK) {
        ...
        if (year < 2024 || year > 2099 || time.tm_mon < 0 || time.tm_mon > 11 || time.tm_mday < 1 || time.tm_mday > 31) {
...
bool settings_get_always_on(void)
{
    return always_on;
}

void settings_set_always_on(bool enabled)
{
    always_on = enabled;
    schedule_save();
}
...
uint8_t settings_get_sound_volume(void)
{
    return sound_volume;
}

void settings_set_sound_volume(uint8_t volume)
{
    if (volume > 100)
        volume = 100;
