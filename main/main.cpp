...
extern "C" void app_main(void)
{
    ...
    display_manager_init();
...

        {
            ...
            std::vector<std::string> ordered_app_names = { "Clock", "Notifications", "Calculator", "2048", "Settings", "Simwatch" };
            ESP_UTILS_CHECK_FALSE_EXIT(
                phone->installAppFromRegistry(inited_apps, &ordered_app_names), "Install app registry failed"
            );


            /* Create a timer to update the clock and battery info */
            lv_timer_create([](lv_timer_t *t)
            {
                ...
                auto *statusBar = phone->getDisplay().getStatusBar();
                ...
                // Обновляем батарею раз в 10 секунд
            ...
            lv_timer_create([](lv_timer_t *t)
            {
                ...
                if (!display_manager_was_woken())
                    return;

                display_manager_clear_woken();

                if (display_manager_was_woken())
                {
                    display_manager_clear_woken();

                    phone->getManager().showApp("Clock");
                }
